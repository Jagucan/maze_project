...
private:
	float _sax, _say, _fs, slice, _d_del, _w_del, far, xfar, yfar, cos,
        sin, cut, xdel, ydel, fstep, angle, sprite, x, y, sfar, sdir;

	int xobj, yobj, xdir, ydir, col, nw, nh, obj, xx, yy, xy, yx,
        cheight, dop, ssize, shor, sver,floor_tile, ceil_tile, pitch = 0, 
        width, height, levelw, levelh, tilew,
		tileh, tileww, tilewh, tileSw, tileSh,
		tileSww, tileSwh, scolkeyr, scolkeyg, scolkeyb,
		renderDataSize = 0, spriteMapSize, renderMax, halfHeight;

	Uint8 _fcr, _fcg, _fcb;

	Uint32 colorkey_conv, ceil_color;

    SDL_Renderer *renderer;

	SDL_Surface *wallsurf, *skysurf, *sprsurf;

    SDL_Texture *surftex, *walls, *Objs;
	...
	float *light_level, *renderDataMem = {};
...
public:
	...
	void setMap(int* sMap_Walls, int sMap_Width, int sMap_Height) {
		level = sMap_Walls;
		levelw = sMap_Width;
		levelh = sMap_Height;
	}
	void setMapObjs(float sSpriteMap[][3], int size) {
		renderDataSize += size;
		spriteMapSize = size;
	...
	void setTileObjs(const char* sTileS, int sTileSw, int sTileSh, int sScolkeyr, int sScolkeyg, int sScolkeyb) {
		tileS = sTileS; // Objs tile
	...
	void init(SDL_Window *window) {
		...
		tileSww = image->w / tileSw; // Objs per width
		tileSwh = image->h / tileSh; // Objs per height
